...
func NewStreamClassReconciler(client client.Client, manager manager.Manager) *StreamClassReconciler {
...
func (s *StreamClassReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	return s.moveFsm(ctx, sc, request.NamespacedName)
}
...
func (s *StreamClassReconciler) moveFsm(ctx context.Context, sc *v1.StreamClass, name types.NamespacedName) (reconcile.Result, error) {
	deleted := sc == nil

	switch {
	case !deleted && sc.Status.Phase == "":
		return s.updatePhase(ctx, sc, name, v1.PhasePending)
	case !deleted && sc.Status.Phase == v1.PhasePending:
		return s.startStreamController(ctx, sc, name, v1.PhaseReady)
	case deleted:
		return s.stopStreamController(ctx, name)
	}

	return reconcile.Result{}, fmt.Errorf("failed to reconcile StreamClass FSM for %s/%s. Current state: %s",
		name.Namespace,
		name.Name,
		sc.StateString(),
	)
...
func (s *StreamClassReconciler) startStreamController(ctx context.Context, sc *v1.StreamClass, name types.NamespacedName, nextPhase v1.Phase) (reconcile.Result, error) {
	reconciler := stream.NewStreamReconciler(sc.TargetResourceGvk(), s.jobBuilder)
	controller, err := reconciler.SetupUnmanaged(s.manager)
	if err != nil {
	...
	go func() {
		...
		// TODO: transition to Failed phase?
	}()
...
func (s *StreamClassReconciler) stopStreamController(ctx context.Context, name types.NamespacedName) (reconcile.Result, error) {
	s.streamControllers[name].cancelFunc()
	// TODO: wait for controller to stop?
